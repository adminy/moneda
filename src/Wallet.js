...
class Wallet extends Component {
  constructor (password, login = '') {
    ...
    this.addresses = []
    this.opened = false
    const db = blockchain.getDb()

    this.data = {}
    this.flush = () => fs.writeFileSync(this.path, Defended.encrypt(Conv.strToBase(Conv.objToJson(this.addresses.map(address => Conv.bufToBase(address.getKeys().priv)))), this.password))
    this.create = () => {
    ...
    this.getBalances = () => {
      ...
      const bals = db.prepare('SELECT address, SUM(amount) amount FROM outs WHERE address IN (' + '?'.repeat(addressRaws.length).split('').join(',') + ') AND inMasterBranch=? AND spentAt=? GROUP BY address').bind(...addressRaws, 1, 0)
    ...
    this.getSoftBalances = () => {
      ...
      const bals = db.prepare('SELECT address, SUM(amount) amount FROM outs WHERE blockHeight>? AND address IN (' + '?'.repeat(addressRaws.length).split('').join(',') + ') AND inMasterBranch=? AND spentAt=? GROUP BY address').bind(blockchainLength - MIN_CONFIRMATIONS, ...addressRaws, 1, 0)
      for (const { address, amount } of bals.iterate()) {
        balances[Address.rawToHash(address)] = amount
      }
      return balances
    ...
    this.sendCoins = (recipients, senders = null) => {
      ...
      for (const address of this.addresses) {
        ...
        if (!senders || senders.includes(addressHash)) {
          const res = db.prepare('SELECT SUM(amount) amount FROM outs WHERE address=? AND inMasterBranch=? AND spentAt=?').get(addressRaw, 1, 0)
      ...
      const createTx = () => {
        ...
        for (const { address, amount, amountm } of recipients) {
        ...
        for (const { address } of addressesBalances) {
          ...
          const outs = db.prepare('SELECT * FROM outs WHERE address=? AND inMasterBranch=? AND spentAt=?').get(addressesData[senderAddress].raw, 1, 0)
...
Wallet.getPath = login => path.join(__dirname, '..', 'data', 'wallet' + (login === '' ? '' : '-' + login) + '.dat')
