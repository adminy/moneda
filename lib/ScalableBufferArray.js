...
class ScalableBufferArray {
  constructor (options) {
    ...
    this.allocIfNeeded = (itemSize) => {
      ...
      diff > 0 && this.alloc(Math.ceil(diff / step))
      return resultSize
    }
    this.allocForWhole = (length) => this.alloc(Math.ceil((length - this.buffer.length) / step))
    this.getFieldStart = (i, name) => {
      const customSizes = this.customSizes[i]
      let start = 0
      for (const fieldName in this.fields) {
        if (fieldName === name) return start
        start += this.fields[fieldName].size || customSizes[fieldName]
      }
    }
    this.getFieldSize = (i, name) => this.fields[name].size || this.customSizes[i][name]
    this.getValue = (data, i, name, type) => {
      const start = this.getFieldStart(i, name)
      const size = this.getFieldSize(i, name)
      if (type === 'number') return data.readUIntBE(start, size)
      else if (type === 'buffer') return data.slice(start, start + size)
    }
  }
  ...
  get (i) {
    ...
    if (end > this.dataSize) return null
