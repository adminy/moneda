...
const { TakeBlocksAfter, BlockFound, BlockFoundZipped, TxInfo, TxInfoZipped } = require('./commands')
...
class Net extends Component {
  constructor () {
    ...
    this.processCommand = (port, address, rawData, callback) => {
      ...
      if (type === Cmd.SRV_INFO && _.size(storage.servers) < 50) {
        ...
        if (data && data.flagZipped) {
          this.log('REQUEST_BLOCKS_AFTER +Z ' + (data.flagAllBranches ? '+' : '-') + 'A')
          const takeBlocksAfter = TakeBlocksAfter.create({ afterHash: data.hash, blockCount: data.count, maxPacketSize: p2x.getMaxMpxSize(), fromAllBranches: data.flagAllBranches })
        ...
        const hash = rawData.slice(1, 33)
        if (storage.session.lastBlockFoundHash && storage.session.lastBlockFoundHash.equals(hash)) {
          // this.log('{red-fg}BLOCK_FOUND IGNORED: Known{/red-fg}')
          return
        }
        storage.session.lastBlockFoundHash = hash
        const blockFound = BlockFound.fromRaw(rawData)
        blockFound.process()
      } else if (type === Cmd.BLOCK_FOUND_ZIPPED) {
        const hash = rawData.slice(1, 33)
        if (storage.session.lastBlockFoundHash && storage.session.lastBlockFoundHash.equals(hash)) {
          // this.log('{red-fg}BLOCK_FOUND_ZIPPED IGNORED: Known{/red-fg}')
          return
        }
        storage.session.lastBlockFoundHash = hash
        const blockFoundZipped = BlockFoundZipped.fromRaw(rawData)
        blockFoundZipped.process()
      } else if (type === Cmd.TX_INFO) {
        const hash = rawData.slice(1, 33)
        if (storage.session.lastTxInfoHash && storage.session.lastTxInfoHash.equals(hash)) {
          // this.log('{red-fg}TX_INFO IGNORED: Known{/red-fg}')
          return
        }
        storage.session.lastTxInfoHash = hash
        const txInfo = TxInfo.fromRaw(rawData)
        txInfo.process()
      } else if (type === Cmd.TX_INFO_ZIPPED) {
        const hash = rawData.slice(1, 33)
        if (storage.session.lastTxInfoHash && storage.session.lastTxInfoHash.equals(hash)) {
          // this.log('{red-fg}TX_INFO_ZIPPED IGNORED: Known{/red-fg}')
          return
        }
        storage.session.lastTxInfoHash = hash
        const txInfoZipped = TxInfoZipped.fromRaw(rawData)
        txInfoZipped.process()
    ...
    p2x.on('online', () => this.broadcastRequestSrvInfo())
    p2x.on('newServer', (port, address, isIpv6) => this.broadcastSrvInfo({ port, address, isIpv6 }))
    p2x.on('rcvdData', (port, address, data) => this.processCommand(port, address, data))
    ...
    p2x.on('mpxRcvdFully', (mpxIdStr, port, address, data, processing, processed) => {
      processing()
      this.processCommand(port, address, data, () => processed())
    })
  }

  sendSrvInfo (port, address, data, callbacks) {
    this.send(port, address, SimpleCommand.create(Cmd.SRV_INFO, data).getRaw())
  }

  sendInfoTakeBlockchainLength (port, address, data) {
    this.send(port, address, SimpleCommand.create(Cmd.INFO_TAKE_BLOCKCHAIN_LENGTH, data).getRaw())
  }
  ...
  broadcastRequestBlocksAfter (hash, count = 4096, id = 0) {
  ...
  broadcastBlockFoundZipped (hash, data) {
    const blockFoundZipped = BlockFoundZipped.create({ hash, data })
    blockFoundZipped.getRaw((raw) => this.broadcast(raw))
  }
  ...
  requestBlocksAfter (hash, count = 4096, responseCallback, finishedCallback) {
    ...
    this.waiters[RESPONSE_REQUEST_BLOCKS_AFTER] = {
      hash,
      timer: setInterval(() => {
        const takeBlocksAfterCount = p2x.getMpxsCountByCmdStr('TAKE_BLOCKS_AFTER')
        if (takeBlocksAfterCount) {
          this.log('Waiting for ' + takeBlocksAfterCount + ' transfer(s)')
        } else {
          clearInterval(this.waiters[RESPONSE_REQUEST_BLOCKS_AFTER].timer)
  ...
  getConstants () { return { RESPONSE_NO_BLOCK, RESPONSE_NO_BLOCK_AFTER } }
}
