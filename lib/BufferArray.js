...
class BufferArray {
  ...
  reverse () {
    const raw = Buffer.allocUnsafe(this.itemSize)
    const half = (this.length >> 1) * this.itemSize
    const last = (this.length - 1) * this.itemSize
    for (let pos = 0; pos < half; pos += this.itemSize) {
      this.buffer.copy(raw, 0, pos)
      this.buffer.copy(this.buffer, pos, last - pos)
      raw.copy(this.buffer, last - pos, 0)
    }
    return true
  }
