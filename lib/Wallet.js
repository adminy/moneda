...
class Wallet extends Component {
  constructor (password, login = '') {
    ...
    this.open = () => {
      ...
      this.addresses = JSON.parse(Buffer.from(decrypted, 'base64')).map(keyBased => Address.fromPrivateKey(Buffer.from(keyBased, 'base64')))
      this.opened = true
      return true
    }
    ...
    this.sendCoins = (recipients, senders = null) => {
      ...
      for (const address of this.addresses) {
        ...
        if (!senders || R.contains(addressHash, senders)) {
          const res = this.db.db.prepare('SELECT SUM(amount) amount FROM outs WHERE address=? AND inMasterBranch=? AND spentAt=?').getAddresses(addressRaw, 1, 0)
