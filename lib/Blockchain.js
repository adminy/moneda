...
module.exports = new class Blockchain extends Component {
  constructor () {
    ...
    this.addBranch = (parentId, length) => {
      const { lastInsertRowid } = this.db.db.prepare('INSERT INTO branches (parentId, isMaster, length) VALUES (?, ?, ?)').run(parentId, 0, length)
    ...
    this.addBlockToBranch = (branchId, height, block) => {
      ...
      if (isBranchMaster && queryUpdateOutsValues.length) {
        ...
        for (const queryUpdateOutsValuesItem of queryUpdateOutsValuesItems) {
          db.prepare('UPDATE outs SET spentAt=? WHERE (' + R.join(' OR ', R.repeat('txHash=? AND outN=?', queryUpdateOutsValuesItem.length >> 1)) + ') AND inMasterBranch=?')
            .run(blockId, ...queryUpdateOutsValuesItem, 1)
        }
      }
    ...
    this.getBlockRowByHash = hash => {
      const blocks = this.db.db.prepare('SELECT * cnt FROM blocks WHERE hash=?').all(hash)
    ...
    this.getBlockCountByHeight = height => this.db.db.prepare('SELECT COUNT(id) cnt FROM blocks WHERE height=?').all(height).map(o => o.cnt)
    this.getBlockRowInBranchStructureByHeight = (branchStructure, height) => {
      const blockRows = this.db.db.prepare('SELECT * FROM blocks WHERE height=? AND branchId IN (' + R.join(',', R.repeat('?', branchStructure.length)) + ') ORDER BY branchId DESC').bind(height, ...branchStructure)
    ...
    this.findBlockRowWithTxInBranchStructure = (hash, branchStructure) => {
      ...
      const blockIdRows = db.prepare('SELECT txs.blockId FROM txs LEFT JOIN blocks ON txs.blockId=blocks.id WHERE txs.hash=? AND blocks.branchId IN (' + R.join(',', R.repeat('?', branchStructure.length)) + ') ORDER BY blocks.branchId DESC, blocks.id ASC').bind(hash, ...branchStructure)
    ...
    this.getCountByTimeInBranchStructure = (branchStructure, since, till) => {
      return this.db.db.prepare('SELECT id, branchId, height, prevBlock, time, hash FROM blocks WHERE time>=? AND time<=?')
        .all(since, till)
    ...
    this.txOutSpentInBranchStructure = (hash, outN, branchStructure) => {
      const spendRows = this.db.db.prepare('SELECT spentAt FROM spends WHERE txHash=? AND outN=?').bind(hash, outN)
