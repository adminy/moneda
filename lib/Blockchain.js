...
module.exports = new class Blockchain extends Component {
  constructor () {
    ...
    this.getBlockRowInBranchStructureByHeight = (branchStructure, height) => {
      const blockRows = db.prepare('SELECT * FROM blocks WHERE height=? AND branchId IN (' + R.join(',', R.repeat('?', branchStructure.length)) + ') ORDER BY branchId DESC').bind(height, ...branchStructure)
